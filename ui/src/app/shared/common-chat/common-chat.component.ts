...
import { ChatResponse, ContentResponse, ContentRetrieved } from 'src/app/constants/model';
...
@Component({
  ...
  styleUrls: ['./common-chat.component.scss']
})
export class CommonChatComponent implements OnInit {

  @Input('web_page_type') web_page_type: string = '';
  loader = false;
  // params!: Params;
  chatForm!: FormGroup;
  ...
  submitQuery(): void {
    ...
    this.loader = true;
      ...
      this.userChat.getQueryResultAll(
        askedQuestion, this.web_page_type
      ).pipe(take(1),
      catchError((error) => {
